...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        \begin{tikzpicture}
            ...
            \node[latent, left=of yhat] (epsi) {$\varepsilon_I$};
            \node[latent, above=of yhat] (alpha) {$\alpha$};
            \node[latent, right=of yhat] (epsj) {$\varepsilon_J$};
            \node[latent, left=of alpha] (sigi) {$\sigma_I$};
            \node[latent, right=of alpha] (sigj) {$\sigma_J$};
            ...
            \plate {I} {(y)(yhat)(epsi)} {$I$};
            \plate {J} {(y)(yhat)(epsj)(I.north east)} {$J$};
    ...
    m = @model I,J begin
        ...
        εi ~ Normal(0, σi) |> iid(I)
        εj ~ Normal(0, σj) |> iid(J)
        yhat = εz .+ εy'
        σ ~ HalfNormal()
        y ~ For(I,J) do i,j 
                Normal(yhat[i,j], σ)
    ...
    \caption{A plate diagram (a) and Soss encoding (b) for a two-way analysis of variance model}
    \label{}
\end{figure}
