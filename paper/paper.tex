...
%   cg(::Type{Lam{Arg, Term}}) where {Arg, Term} =  :($Arg -> $(cg(Term)))
%   cg(::Type{App{Fn, Arg}}) where {Fn, Arg} =  :($(cg(Fn))($(cg(Arg))))
%   cg(::Type{Var{N}}) where N = N

%   @gg lc(::Type{Term}) where Term <: LCTerm = cg(Term)
% \end{minted} 
  % tolc(expr::Expr) =  @match expr begin
  %                       :($f($arg)) => App{tolc(f), tolc(arg)}
  %                       :($x => $y) => Lam{x, tolc(y)}
  %                     end
  % tolc(sym::Symbol) => Var{sym}
  % macro lc(term)
  %   lc(tolc(term))
  % end
